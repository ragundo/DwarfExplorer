...
void fill_globals(Node* p_node_parent)
{
	...
	field_name = "cur_rain";
	auto n_cur_rain            = new NodeArray;
	n_cur_rain->m_address      = reinterpret_cast<uint64_t>(df::global::cur_rain);
	n_cur_rain->m_field_name   = field_name;
	n_cur_rain->m_df_type      = DF_Type::coord;
	n_cur_rain->m_rdf_type     = RDF_Type::Array;
	n_cur_rain->m_node_type    = NodeType::Array;
	n_cur_rain->m_defined_in   = "df.map.xml";
	n_cur_rain->m_addornements = "[1280";
	n_cur_rain->m_array_size   = 1280;
	n_cur_rain->m_address      = reinterpret_cast<uint64_t>(df::global::cur_rain);
	n_cur_rain->m_parent       = p_node_parent;
	p_node_parent->m_children.push_back(n_cur_rain);

	field_name = "cur_rain_counter";
	auto n_cur_rain_counter          = new NodeSimple<int16_t>;
	n_cur_rain_counter->m_field_name = field_name;
	n_cur_rain_counter->m_df_type    = DF_Type::int16_t;
	n_cur_rain_counter->m_rdf_type   = RDF_Type::int16_t;
	n_cur_rain_counter->m_node_type  = NodeType::Simple;
	n_cur_rain_counter->m_address    = reinterpret_cast<uint64_t>(df::global::cur_rain_counter);
	n_cur_rain_counter->m_parent     = p_node_parent;
	p_node_parent->m_children.push_back(n_cur_rain_counter);
	...
	field_name = "cur_snow";
	auto n_cur_snow            = new NodeArray;
	n_cur_snow->m_address      = reinterpret_cast<uint64_t>(df::global::cur_snow);
	n_cur_snow->m_field_name   = field_name;
	n_cur_snow->m_df_type      = DF_Type::coord;
	n_cur_snow->m_rdf_type     = RDF_Type::Array;
	n_cur_snow->m_node_type    = NodeType::Array;
	n_cur_snow->m_defined_in   = "df.map.xml";
	n_cur_snow->m_addornements = "[256";
	n_cur_snow->m_array_size   = 256;
	n_cur_snow->m_address      = reinterpret_cast<uint64_t>(df::global::cur_snow);
	n_cur_snow->m_parent       = p_node_parent;
	p_node_parent->m_children.push_back(n_cur_snow);

	field_name = "cur_snow_counter";
	auto n_cur_snow_counter          = new NodeSimple<int16_t>;
	n_cur_snow_counter->m_field_name = field_name;
	n_cur_snow_counter->m_df_type    = DF_Type::int16_t;
	n_cur_snow_counter->m_rdf_type   = RDF_Type::int16_t;
	n_cur_snow_counter->m_node_type  = NodeType::Simple;
	n_cur_snow_counter->m_address    = reinterpret_cast<uint64_t>(df::global::cur_snow_counter);
	n_cur_snow_counter->m_parent     = p_node_parent;
	p_node_parent->m_children.push_back(n_cur_snow_counter);
	...
	field_name = "handleannounce";
	auto n_handleannounce          = new NodeSimple<bool>;
	n_handleannounce->m_field_name = field_name;
	n_handleannounce->m_df_type    = DF_Type::Bool;
	n_handleannounce->m_rdf_type   = RDF_Type::Bool;
	n_handleannounce->m_node_type  = NodeType::Simple;
	n_handleannounce->m_address    = reinterpret_cast<uint64_t>(df::global::handleannounce);
	n_handleannounce->m_parent     = p_node_parent;
	p_node_parent->m_children.push_back(n_handleannounce);
	...
	field_name = "jobvalue";
	auto n_jobvalue            = new NodeArray;
	n_jobvalue->m_address      = reinterpret_cast<uint64_t>(df::global::jobvalue);
	n_jobvalue->m_field_name   = field_name;
	n_jobvalue->m_df_type      = DF_Type::int32_t;
	n_jobvalue->m_rdf_type     = RDF_Type::Array;
	n_jobvalue->m_node_type    = NodeType::Array;
	n_jobvalue->m_addornements = "[260";
	n_jobvalue->m_array_size   = 260;
	n_jobvalue->m_address      = reinterpret_cast<uint64_t>(df::global::jobvalue);
	n_jobvalue->m_parent       = p_node_parent;
	p_node_parent->m_children.push_back(n_jobvalue);

	field_name = "jobvalue_setter";
	auto n_jobvalue_setter            = new NodeArray;
	n_jobvalue_setter->m_address      = reinterpret_cast<uint64_t>(df::global::jobvalue_setter);
	n_jobvalue_setter->m_field_name   = field_name;
	n_jobvalue_setter->m_df_type      = DF_Type::unit;
	n_jobvalue_setter->m_rdf_type     = RDF_Type::Array;
	n_jobvalue_setter->m_node_type    = NodeType::Array;
	n_jobvalue_setter->m_defined_in   = "df.units.xml";
	n_jobvalue_setter->m_addornements = "[260";
	n_jobvalue_setter->m_array_size   = 260;
	n_jobvalue_setter->m_address      = reinterpret_cast<uint64_t>(df::global::jobvalue_setter);
	n_jobvalue_setter->m_parent       = p_node_parent;
	p_node_parent->m_children.push_back(n_jobvalue_setter);
	...
	field_name = "preserveannounce";
	auto n_preserveannounce          = new NodeSimple<bool>;
	n_preserveannounce->m_field_name = field_name;
	n_preserveannounce->m_df_type    = DF_Type::Bool;
	n_preserveannounce->m_rdf_type   = RDF_Type::Bool;
	n_preserveannounce->m_node_type  = NodeType::Simple;
	n_preserveannounce->m_address    = reinterpret_cast<uint64_t>(df::global::preserveannounce);
	n_preserveannounce->m_parent     = p_node_parent;
	p_node_parent->m_children.push_back(n_preserveannounce);
	...
	field_name = "updatelightstate";
	auto n_updatelightstate          = new NodeSimple<bool>;
	n_updatelightstate->m_field_name = field_name;
	n_updatelightstate->m_df_type    = DF_Type::Bool;
	n_updatelightstate->m_rdf_type   = RDF_Type::Bool;
	n_updatelightstate->m_node_type  = NodeType::Simple;
	n_updatelightstate->m_address    = reinterpret_cast<uint64_t>(df::global::updatelightstate);
	n_updatelightstate->m_parent     = p_node_parent;
	p_node_parent->m_children.push_back(n_updatelightstate);
	...
	field_name = "weathertimer";
	auto n_weathertimer          = new NodeSimple<int16_t>;
	n_weathertimer->m_field_name = field_name;
	n_weathertimer->m_df_type    = DF_Type::int16_t;
	n_weathertimer->m_rdf_type   = RDF_Type::int16_t;
	n_weathertimer->m_node_type  = NodeType::Simple;
	n_weathertimer->m_address    = reinterpret_cast<uint64_t>(df::global::weathertimer);
	n_weathertimer->m_parent     = p_node_parent;
	p_node_parent->m_children.push_back(n_weathertimer);
