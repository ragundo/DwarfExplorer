...
DF_Type get_df_subtype(DF_Type p_base_type, uint64_t p_address)
{
    switch(p_base_type)
    {
        case DF_Type::text_info_elementst:
            return text_info_elementst_subtype(p_address);
        case DF_Type::adventure_optionst:
            return adventure_optionst_subtype(p_address);
        case DF_Type::adventure_movement_optionst:
            return adventure_movement_optionst_subtype(p_address);
        case DF_Type::adventure_item_interact_choicest:
            return adventure_item_interact_choicest_subtype(p_address);
        case DF_Type::art_image_element:
            return art_image_element_subtype(p_address);
        case DF_Type::art_image_property:
            return art_image_property_subtype(p_address);
        case DF_Type::building_def:
            return building_def_subtype(p_address);
        case DF_Type::building:
            return building_subtype(p_address);
        ...
        case DF_Type::mental_picture_elementst:
            return mental_picture_elementst_subtype(p_address);
        case DF_Type::history_event:
            return history_event_subtype(p_address);
        case DF_Type::history_event_collection:
            return history_event_collection_subtype(p_address);
        case DF_Type::interaction_effect:
            return interaction_effect_subtype(p_address);
        case DF_Type::interaction_source:
            return interaction_source_subtype(p_address);
        case DF_Type::interaction_target:
            return interaction_target_subtype(p_address);
        case DF_Type::itemdef:
            return itemdef_subtype(p_address);
        case DF_Type::itemimprovement:
            return itemimprovement_subtype(p_address);
        case DF_Type::spatter_common:
            return spatter_common_subtype(p_address);
        case DF_Type::item:
            return item_subtype(p_address);
        case DF_Type::machine:
            return machine_subtype(p_address);
        case DF_Type::block_square_event:
            return block_square_event_subtype(p_address);
        case DF_Type::feature:
            return feature_subtype(p_address);
        case DF_Type::feature_init:
            return feature_init_subtype(p_address);
        case DF_Type::feature_alteration:
            return feature_alteration_subtype(p_address);
        case DF_Type::world_construction_square:
            return world_construction_square_subtype(p_address);
        case DF_Type::world_construction:
            return world_construction_subtype(p_address);
        case DF_Type::flow_guide:
            return flow_guide_subtype(p_address);
        case DF_Type::region_block_eventst:
            return region_block_eventst_subtype(p_address);
        case DF_Type::material_common:
            return material_common_subtype(p_address);
        case DF_Type::script_stepst:
            return script_stepst_subtype(p_address);
        case DF_Type::script_varst:
            return script_varst_subtype(p_address);
        case DF_Type::active_script_varst:
            return active_script_varst_subtype(p_address);
        case DF_Type::activity_event:
            return activity_event_subtype(p_address);
        case DF_Type::squad_order:
            return squad_order_subtype(p_address);
        case DF_Type::projectile:
            return projectile_subtype(p_address);
        case DF_Type::reaction_reagent:
            return reaction_reagent_subtype(p_address);
        case DF_Type::reaction_product:
            return reaction_product_subtype(p_address);
        case DF_Type::general_ref:
            return general_ref_subtype(p_address);
        case DF_Type::histfig_entity_link:
            return histfig_entity_link_subtype(p_address);
        case DF_Type::histfig_site_link:
            return histfig_site_link_subtype(p_address);
        case DF_Type::histfig_hf_link:
            return histfig_hf_link_subtype(p_address);
        case DF_Type::resource_allotment_specifier:
            return resource_allotment_specifier_subtype(p_address);
        case DF_Type::creature_interaction_effect:
            return creature_interaction_effect_subtype(p_address);
        case DF_Type::build_req_choicest:
            return build_req_choicest_subtype(p_address);
        case DF_Type::interface_button:
            return interface_button_subtype(p_address);
        case DF_Type::viewscreen:
            return viewscreen_subtype(p_address);
        case DF_Type::layer_object:
            return layer_object_subtype(p_address);
        case DF_Type::world_gen_param_basest:
            return world_gen_param_basest_subtype(p_address);
        case DF_Type::abstract_building:
            return abstract_building_subtype(p_address);
        case DF_Type::site_realization_building_infost:
            return site_realization_building_infost_subtype(p_address);
        case DF_Type::creation_zone_pwg_alterationst:
            return creation_zone_pwg_alterationst_subtype(p_address);
        default:
            break;
    }
    return DF_Type::None;
